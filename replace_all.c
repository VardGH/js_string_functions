...
char* replace_all(const char *str, const char *pattern, const char *replacement)
{
    char *result = replace(str, pattern, replacement);
    int index = index_of(result, pattern);

    while (index != -1) {
        result = replace(result, pattern, replacement);
        index = index_of(result, pattern);
    }
...
int main()
{
    ...
    const char *replacement = "cat";

    char *result = replace_all(str, pattern, replacement);
    if (result != NULL) {
